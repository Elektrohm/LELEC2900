...
INPUTS: 
    ...
    - (file_path) : path where the graph must be saved (if needed)
--------------------------------------------------------------------------------------------------"""
def plot_dft(signals, fft, legend, show_fig, lim=[None,None], file_path=None): 
    # One big figure to frame the whole
    fig = plt.figure(figsize=(12,3))
    ax1 = fig.add_subplot(121) 
    ...
    ax2 = fig.add_subplot(122) 
    for idx, color, label in zip(range(1, fft.shape[0]), colors2, legend):
        ax2.stem(fft[0], fft[idx], label= label)
        ax2.set_xlim(lim)
        ax2.set_xlabel('Frequency (Hz)')
        ax2.legend(bbox_to_anchor=(1.04,1), loc="upper left") 
...
INPUTS: 
    ...
    - (file_path) : path where the graph must be saved (if needed)
--------------------------------------------------------------------------------------------------"""
def plot_window(win, fft, legend, show_fig, lim=[None,None], file_path=None): 
    # One big figure to frame the whole
